...
public class DocumentController {
    ...
    public DocumentController(DocumentService documentService, RequestUserContext requestUserContext, ObjectMapper objectMapper) {
    ...
    public ResponseEntity<DocumentResponse> uploadDocument(@Valid @RequestPart("metadata") DocumentUploadMetadata metadata,
    ...
    public ResponseEntity<DocumentResponse> updateStatus(@PathVariable Long id, @Valid @RequestBody UpdateStatusRequest request) {
        RequestUser user = requestUserContext.requireUser();
        DocumentResponse response = documentService.updateStatus(id, request.getStatus(), user, "STATUS_UPDATE", request.getComment());
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{id}/metadata")
    public ResponseEntity<DocumentResponse> updateMetadata(@PathVariable Long id,
    ...
    public ResponseEntity<List<AuditEntryResponse>> getAuditTrail(@PathVariable Long id) {
        ...
        List<AuditEntryResponse> response = entries.stream().map(this::mapAuditLog).collect(Collectors.toList());
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{id}/rework")
    public ResponseEntity<DocumentResponse> markForRework(@PathVariable Long id,
    ...
    @PutMapping("/{id}/close")
    public ResponseEntity<DocumentResponse> close(@PathVariable Long id,
                                                   @Valid @RequestBody(required = false) DocumentActionRequest request) {
        RequestUser user = requestUserContext.requireUser();
        String comment = request != null ? request.getComment() : null;
    ...
    public ResponseEntity<DocumentResponse> moveToUnderReview(@PathVariable Long id,
                                                               @Valid @RequestBody(required = false) DocumentActionRequest request) {
        RequestUser user = requestUserContext.requireUser();
        String comment = request != null ? request.getComment() : null;
